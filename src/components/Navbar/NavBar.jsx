import React, { useState } from 'react'
import { NavLink } from 'react-router-dom'

export default function NavBar() {
  const [isActive, setActive] = useState(false);

  return (
    <>
      {
        isActive ? <div className='overlay fixed w-full top-0 bottom-0  bg-black/50' onClick={() => setActive(false)}>
          <div className='fixed top-0 right-0 text-white mt-5 me-5'>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" className='w-6 h-6'>
              <path fill='white' d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"/>
            </svg>
          </div>
        </div> : null
      }
      <nav className={`${isActive ? "bg-white fixed top-0 bottom-0 left-0 py-10 flex flex-col items-start gap-y-6 w-1/2" : "hidden w-fit sm:flex sm:flex-row gap-x-2"}`}>
        <NavLink to="home" className="nav-link">Home</NavLink>
        <NavLink to="" className="nav-link">Courts</NavLink>
        <NavLink to="" className="nav-link">Prices</NavLink>
        <NavLink to="" className="nav-link">About</NavLink>
        <NavLink to="" className="nav-link">Contact us</NavLink>
        <NavLink to="" className="nav-link">Register/Login</NavLink>
      </nav>
      <div className='sm:hidden' onClick={() => setActive(true)}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" className="w-6 h-6">
          <path d="M0 96C0 78.3 14.3 64 32 64l384 0c17.7 0 32 14.3 32 32s-14.3 32-32 32L32 128C14.3 128 0 113.7 0 96zM0 256c0-17.7 14.3-32 32-32l384 0c17.7 0 32 14.3 32 32s-14.3 32-32 32L32 288c-17.7 0-32-14.3-32-32zM448 416c0 17.7-14.3 32-32 32L32 448c-17.7 0-32-14.3-32-32s14.3-32 32-32l384 0c17.7 0 32 14.3 32 32z" />
        </svg>
      </div>
    </>
  )
}
